...
class BP_Toolkit {
    ...
    public function bptk_init() {
        ...
        $this->report_init();
        // Initialise our 'moderation' function
        $this->moderation_init();
        $this->report_settings_init();
        // Initialise our reports screen function
        $this->report_screen_init();
    }
    ...
    private function load_dependencies() {
        ...
        /**
         * The class responsible for defining all actions that occur in the reports screen area.
         */
        require_once plugin_dir_path( dirname( __FILE__ ) ) . 'admin/class-bp-toolkit-reports-screen.php';
        /**
        ...
        /**
         * The class responsible for defining the Toolkit's additional moderation functionality
         */
        require_once plugin_dir_path( dirname( __FILE__ ) ) . 'includes/class-bp-toolkit-moderate.php';
        require_once plugin_dir_path( dirname( __FILE__ ) ) . 'admin/class-bp-toolkit-report-settings.php';
        require_once plugin_dir_path( dirname( __FILE__ ) ) . 'includes/class-bp-toolkit-report.php';
    ...
    private function set_locale() {
        load_plugin_textdomain( 'bp-toolkit', false, dirname( dirname( plugin_basename( __FILE__ ) ) ) . '/languages/' );
    }
    ...
    private function define_admin_hooks() {
        ...
        $this->loader->add_action( 'init', $plugin_admin, 'add_report_caps' );
        ...
        $this->loader->add_action( 'admin_notices', $plugin_admin, 'handle_admin_notices' );
        ...
        $this->loader->add_action( 'wp_ajax_toggle_uphold', $plugin_admin, 'toggle_uphold' );
        $this->loader->add_action( 'wp_ajax_toggle_suspension', $plugin_admin, 'toggle_suspension' );
        $this->loader->add_action( 'wp_ajax_toggle_moderation', $plugin_admin, 'toggle_moderation' );
        $this->loader->add_action( 'wp_ajax_quick_moderate', $plugin_admin, 'quick_moderate' );
        $this->loader->add_action( 'wp_ajax_rebuild_blocks', $plugin_admin, 'rebuild_blocks' );
        $this->loader->add_action(
            'wp_dashboard_setup',
            $plugin_admin,
        ...
        // $this->loader->add_action( 'admin_notices', $plugin_admin, 'subscriber_check_activation_notice' );
    }
    ...
    private function define_public_hooks() {
        ...
        $this->loader->add_action( 'template_redirect', $plugin_public, 'filter_frontend_reports' );
    }
    ...
    /**
     * Instantiate additional moderation functionality of the plugin.
     */
    private function moderation_init() {
        $moderation = new BPTK_Moderate();
    }

    /**
     * Instantiate the class that controls our report settings.
     *
     * @since    3.0.0
     * @access   private
     */
    public function report_settings_init() {
        $report = new BPTK_Report_Settings();
    }

    /**
     * Populate a reports CPT edit screen.
     *
     * @since    2.1.0
     * @access   private
     */
    public function report_screen_init() {
        $report = new BP_Toolkit_Admin_Report_Screen($this->get_bp_toolkit(), $this->get_version());
        $report->init();
    }
