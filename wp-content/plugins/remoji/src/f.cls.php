...
class f {
	...
	public static function save( $filename, $data, $mkdir = false, $append = false, $silence = true ) {
		...
		if ( ! file_exists( $folder ) ) {
			if ( ! $mkdir ) {
				return $silence ? false : sprintf( __( 'Folder does not exist: %s', 'remoji' ), $folder );
			}
			...
			try {
				...
				return $silence ? false : sprintf( __( 'Can not create folder: %1$s. Error: %2$s', 'remoji' ), $folder, $ex->getMessage() );
			}
		}

		if ( ! file_exists( $filename ) ) {
			if ( ! is_writable( $folder ) ) {
				return $silence ? false : sprintf( __( 'Folder is not writable: %s.', 'remoji' ), $folder );
			}
			try {
				touch( $filename );
			}
			catch ( \Exception $ex ){
				return $silence ? false : sprintf( __( 'File %s is not writable.', 'remoji' ), $filename );
			}
		}
		...
		if ( $ret === false ) {
			return $silence ? false : sprintf( __( 'Failed to write to %s.', 'remoji' ), $filename );
		}
